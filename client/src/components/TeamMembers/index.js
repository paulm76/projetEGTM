      ...
      <div style={{width: this.props.width, margin: 'auto', display:'flex'}}>
        {this.props.members.map(member =>
          ...
          {member.id==this.props.idadmin && <text x={40*this.props.ratio} y={26*this.props.ratio} stroke="white" stroke-width={1.5*this.props.ratio} fill="#ffd000"  style={{fontSize:32*this.props.ratio}}>★</text>}
            {member.Places_prises>1 && <svg><circle cx={52*this.props.ratio} cy={44*this.props.ratio} r={11*this.props.ratio} fill="#0073ad" stroke="white" stroke-width={3*this.props.ratio} />
           <text x={44*this.props.ratio} y={48*this.props.ratio} fill="white" style={{fontSize:13*this.props.ratio, fontWeight:'bold'}}>+{member.Places_prises-1}</text></svg>}
